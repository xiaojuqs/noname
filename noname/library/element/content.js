...
export const Content = {
			...
			var dragEnd = function(e) {
				...
				buttonss.forEach(btn => {
					Array.from(btn.children).forEach(element => {
						...
						element.style.transform = 
							`translateX(${(element._rect.left - element._rect2.left) / game.documentZoom}px) translateY(${(element._rect.top - element._rect2.top) / game.documentZoom}px)`;
					});
				});
				// play
				...
				setTimeout(() => {
					Promise.race([
						...
						Promise.all(buttonss.map(btn => Array.from(btn.children)).flat(1).map(element => {
	...
	chooseDrawRecover: function () {
		...
		if (event.ai) {
			...
			var choice;
			if (player.isDamaged() && get.recoverEffect(player) > 0 && (player.hp == 1 || player.needsToDiscard() || player.hasSkillTag("maixie_hp") || event.num2 > event.num1 || (event.num2 == event.num1 && player.needsToDiscard(1)))) {
				choice = "recover_hp";
			} else {
				choice = "draw_card";
			}
