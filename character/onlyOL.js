...
game.import('character',function(lib,game,ui,get,ai,_status){
	return {
		...
		character:{
			...
			ol_chengpu:['male','wu',4,['ol_lihuo','olchunlao'],['tempname:xin_chengpu','die_audio:xin_chengpu']],
		},
		characterSort:{
			onlyOL:{
				...
				subSkill:{
					add:{
						...
						async content(event,trigger,player){
							const {result:{bool,targets}}=await player.chooseTarget(get.prompt('ol_lihuo'),'为'+get.translation(trigger.card)+'增加一个目标',(card,player,target)=>{
							...
							if(bool){
								player.logSkill('ol_lihuo',targets);
		...
		translate:{
			...
			ol_lihuo:'疠火',
			ol_lihuo_info:'①你使用的非火【杀】可以改为火【杀】，若如此做，此牌结算完毕后，若此牌造成过伤害，则你弃置一张牌或失去1点体力。②你使用火【杀】可以额外指定一个目标。',
			olchunlao:'醇醪',
