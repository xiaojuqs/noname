		...
		skill:{
			...
			nzry_feijun:{
				...
				ai:{
					...
					result:{
						player:function(player){
							if(game.countPlayer(function(current){return (current.countCards('h')>player.countCards('h')||current.countCards('e')>player.countCards('e'))&&get.attitude(player,current)<0&&!player.getStorage('nzry_feijun').includes(current)})>0||game.countPlayer(function(current){return current.countCards('h')>player.countCards('h')&&get.attitude(player,current)<0})>0||(player.countCards('h')>=2&&game.countPlayer(function(current){return current.countCards('e')>player.countCards('e')&&get.attitude(player,current)<0})>0)) return 1;
						},
					},
				},
			},
				...
				ai:{
					...
					effect:{
						target:function(card,player,target){
							...
							if(target.hp===2&&get.tag(card,'damage')==1&&!target.isTurnedOver()&&
