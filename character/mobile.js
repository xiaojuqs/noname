...
game.import('character',function(lib,game,ui,get,ai,_status){
	return {
		...
		characterSort:{
			mobile:{
				mobile_default:["miheng","taoqian","lingcao","sunru","lifeng","zhuling","liuye","zhaotongzhaoguang","majun","simazhao","wangyuanji","pangdegong","shenpei","hujinding","zhangyì","jiakui","yangbiao","chendeng","dongcheng","yangyi","dengzhi","zhengxuan","sp_sufei","furong","dingyuan","simashi","yanghuiyu","hucheer","gongsunkang","nanhualaoxian","zhouqun","qiaozhou","fuqian","simafu","mayuanyi","yanpu"],
		...
		character:{
			sp_huaman:['female','shu',3,['spxiangzhen','spfangzong','spxizhan']],
		...
		skill:{
			...
			yiyong:{
				...
				content:function(){
					...
					}).length==cards.length&&player.canUse({name:'sha',cards:cards},target,false)) player.useCard({name:'sha'},cards,target,false);
				},
			},
			shanxie:{
				audio:2,
				...
				content:function(){
					...
					if(card) player.gain(card,'gain2');
						...
						if(targets.length){
							...
							player.gain(target,target.getEquip(1),'give');
						}
					}
				},
				ai:{
			...
			disordersidi:{
				...
				subSkill:{
					clear:{
						...
						locked:false,
						filter:function(event,player){
							if(!player.storage.disordersidi||!player.storage.disordersidi.contains(event.player)) return false;
							if(event.name=='die') return true;
							if(get.type(event.card,false)!='delay'){
								var index=player.storage.disordersidi.indexOf(event.player);
								return index!=-1&&(player.storage.disordersidi2[index]!=event.target||event.targets.length!=1);
							}
							return false;
						},
						content:function(){
			...
			zhengjing:{
				...
				content:function(){
					...
					var next=player.chooseToMove('整经：请分配整理出的经典');
					next.set('list',[
						['置于'+get.translation(target)+'的武将牌上',cards],
						['自己获得'],
					]);
					next.set('filterMove',function(from,to,moved){
						if(moved[0].length==1&&from==moved[0][0]) return false;
						return true;
					});
		...
		translate:{
			...
			wujing:'吴景',
			heji:'合击',
			heji_info:'当有角色使用的【决斗】或红色【杀】结算完成后，若此牌对应的目标数为1，则你可以对相同的目标使用一张【杀】或【决斗】（无距离和次数限制）。若你以此法使用的牌不为转化牌，则你从牌堆中随机获得一张牌。',
			liubing:'流兵',
			...
			xin_caozhen:'手杀曹真',
			disordersidi:'司敌',
			disordersidi_info:'①当你使用的不为延时锦囊牌的牌结算结束后，你可选择一名R内不存在以a为第一序偶的二元序偶的其他角色a，并选择一名角色b，在关系R内建立二元序偶<a,b>（b对其他角色不可见）。②一名角色a使用不为延时锦囊牌的牌指定b为目标时，若(aRb)∧(此牌目标数为1)为真，则{你从R内移除<a,b>，且：若b为你，你摸一张牌；若b不为你，你可选择：⒈取消此牌的目标，然后若场上没有处于濒死状态的角色，则你对a造成1点伤害。⒉摸两张牌}；否则{你清除R内以a为第一元素的二元序偶}。',
			sp_chendong:'陈武董袭',
			spyilie:'毅烈',
			...
			sp_wangshuang:'手杀王双',
			yiyong:'异勇',
			yiyong_info:'当你受到其他角色造成的渠道为【杀】的伤害后，若你的装备区内有武器牌，则你可以获得此【杀】对应的所有实体牌，然后将这些牌当做【杀】对伤害来源使用（无距离限制）。',
			shanxie:'擅械',
			shanxie_info:'①出牌阶段限一次，你可以从牌堆中获得一张武器牌（没有则改为从场上随机获得一张）。②当其他角色使用【闪】响应你使用的【杀】时，若此【闪】没有点数或点数不大于你攻击范围的二倍，则你令此【闪】无效。',
			sunyi:'孙翊',
			zaoli:'躁厉',
			zaoli_info:'锁定技。①你不能于回合内使用你手牌中不为本回合获得的牌。②当你使用或打出手牌时，你获得一个“厉”（至多4个）。③回合开始时，若你有“厉”，则你移去所有“厉”并弃置任意张牌，然后摸X+Y张牌并失去1点体力（X为你移去的标记数，Y为你弃置的牌数）。',
			sp_gaolan:'手杀高览',
			spjungong:'峻攻',
			spjungong_info:'出牌阶段，你可失去X+1点体力或弃置X+1张牌，视为对一名其他角色使用【杀】（不计入次数和距离限制）。若你因此【杀】造成了伤害，则你令此技能失效直到回合结束。',
			spdengli:'等力',
			...
			spfangzong:'芳踪',
			spfangzong_info:'锁定技。若你于当前回合内未发动过〖嬉战〗选择过选项二，则：①你不能于回合内使用具有伤害标签的牌指定攻击范围内的角色为目标。②攻击范围内包含你的角色不能使用是呀具有伤害标签的牌指定你为目标。③结束阶段，你将手牌摸至X张（X为场上存活人数且至多为8）',
			spxizhan:'嬉战',
