	...
	return {
		...
		skill:{
					...
					eff0:{
						...
						content:function(){
							...
							if(player.storage.twzhengjian){
								player.chooseBool('征建：是否对'+get.translation(target)+'造成1点伤害？').set('ai',()=>_status.event.goon).set('goon',get.damageEffect(target,player,_status.event.player)>0&&get.attitude(target,player)<0);
							}
							else{
			...
			twhengjiang:{
				...
				trigger:{player:'useCardToPlayer'},
				filter:function(event,player){
					return !player.hasSkill('twhengjiang2')&&event.targets.length==1&&['basic','trick'].includes(get.type(event.card,false))&&player.isPhaseUsing()&&game.hasPlayer(function(current){
