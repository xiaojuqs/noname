...
game.import("character", function () {
	return {
			...
			twxiayong: {
				...
				subSkill: {
					effect: {
						...
						popup: false,
						async content(event, trigger, player) {
							await player.logSkill(
								"twxiayong" + (trigger.player === player ? "1" : "2"),
								trigger.player
							);
					...
					for (var current of game.players) {
						...
						for (var card of player.getCards("he")) {
							...
							if (
								...
								!ui.selected.cards.some((i) => get.subtype(i) == "equip1")
							) {
						...
						if ((weapon && player.hp > 2) || get.damageEffect(player, current, player) > 10)
							list[1].push(current);
					}
					list[0].sort((a, b) => {
			...
			twmouli: {
				...
				ai: {
					...
					skillTagFilter: function (player, tag, arg) {
						...
						for (var i = 0; i < ui.cardPile.childNodes.length; i++) {
							var card = ui.cardPile.childNodes[i];
							if (get.type(card, player) == "basic" && !list.includes(card.name))
								list.push(card.name);
						}
							...
							event.target
								...
								.set(
									...
									(function () {
										...
										var d1 = true;
										if (
											trigger.player.hasSkill("jueqing") ||
											trigger.player.hasSkill("gangzhi")
										)
											d1 = false;
			...
			twhuzhu: {
				...
				ai: {
					...
					result: {
						target: function (player, target) {
							var eff =
								target.isDamaged() && get.attitude(target,player)>0
		...
		translate: {
			...
			twfenwang: "焚亡",
			twfenwang_info:
				"锁定技。①当你受到属性伤害时，你须弃置一张牌或令此伤害+1。②当你对其他角色造成非属性伤害时，若你的手牌数大于等于其，则此伤害+1。",
			xia_xiahousone: "夏侯子萼",
			twchengxi: "承袭",
			twchengxi_info:
			...
			twchue: "除恶",
			twchue_info:
				"①当你使用【杀】指定唯一目标时，若场上存在可称为此【杀】目标的非目标角色，则你可以失去1点体力，为此牌额外指定Z个目标。②当你受到伤害或失去体力后，你摸一张牌并获得1个“勇”标记。③回合结束时，若你的“勇”标记数大于等于Z，则你可以失去Z个“勇”标记，视为使用一张伤害+1且可以额外指定Z个目标的【杀】。（Z为你的体力值）",
			xia_shitao: "石韬",
			twjieqiu: "劫囚",
			twjieqiu_info:
