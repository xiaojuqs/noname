	...
	return {
		...
		characterSort:{
			sp2:{
				...
				sp_qihuan:['zhaozhong','re_hejin','fengfang'],
				sp_binglin:['re_niujin',"sp_mifangfushiren",'licaiwei'],
				sp_danqi:['dufuren','qinyilu','bianxi'],
				...
				sp_zhilan:['liuyong','wanniangongzhu','zhanghu','lvlingqi','tenggongzhu'],
				sp_guixin:['re_kanze','re_chendeng','caimaozhangyun','dc_lvkuanglvxiang','dc_gaolan'],
				sp_daihan:['mamidi','dc_jiling','zhangxun'],
				...
				sp2_shengun:["puyuan","guanlu","gexuan",'wufan','re_zhangbao','dukui'],
				sp2_bizhe:['dc_luotong','dc_wangchang'],
				...
				sp_decade:['huaman','caobuxing','re_maliang','xin_baosanniang','re_xinxianying','dongxie','zhouyi','dc_jiben','zhaoang','dc_liuba','liuhui','guanhai','quanhuijie','yinfuren','dc_huangquan','dc_huban','chengui','dingshangwan'],
			}
		},
		skill:{
			//丁尚涴
			...
			dcyingtu:{
				...
				filter:function(event,player){
					...
					if(target!=player.getNext()&&target!=player.getPrevious()) return false;
				...
				check:function(event,player){
					...
					var target=(event.player==player.getNext()?player.getPrevious():player.getNext());
			...
			dcchongyi:{
				...
				subSkill:{
					...
					keep:{
						charlotte:true,
						mod:{
							cardUsable:function(player,num){
			...
			dcyingyu:{
				...
				content:function(){
					...
					if(result.control!='cancel2'){
						var i=result.index;
						targets[i].gain(cards[1-i],targets[1-i],'give');
					}
				},
				onremove:true,
			},
			...
			dcshuhe:{
				...
				content:function(){
					...
					if(lose_list.length){
						...
						event.goto(3);
						player.chooseTarget(true,lib.filter.notMe,'令一名其他角色获得'+get.translation(event.cards[0])).set('ai',function(target){
							return get.attitude(_status.event.player,target);
						});
					}
					'step 2'
		...
		translate:{
			...
			chengui:'陈珪',
			dcyingtu:'营图',
			dcyingtu_info:'每回合限一次。当你的上家/下家于出牌阶段外获得牌后，你可以获得其一张牌，然后将一张牌交给你的下家/上家。若你给出的牌为装备牌，则其使用之。',
			dccongshi:'从势',
