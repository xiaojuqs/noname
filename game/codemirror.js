(() => {
  ...
  (function (mod) {
    ...
    function setDocumentHeight(cm, measure) {
      var documentZoom = getDocumentZoom();
      cm.display.sizer.style.minHeight = measure.docHeight / documentZoom + "px";
      cm.display.heightForcer.style.top = measure.docHeight / documentZoom + "px";
      cm.display.gutters.style.height = (measure.docHeight + cm.display.barHeight + scrollGap(cm)) / documentZoom + "px";
    }
    ...
    function drawSelectionCursor(cm, head, output) {
      ...
      cursor.style.left = pos.left / documentZoom + "px";
      cursor.style.top = pos.top / documentZoom + "px";
      cursor.style.height = Math.max(0, pos.bottom - pos.top) * cm.options.cursorHeight / documentZoom + "px";

      if (pos.other) {
        // Secondary cursor, shown when on a 'jump' in bi-directional text
        var otherCursor = output.appendChild(elt("div", "\u00a0", "CodeMirror-cursor CodeMirror-secondarycursor"));
        otherCursor.style.display = "";
        otherCursor.style.left = pos.other.left / documentZoom + "px";
        otherCursor.style.top = pos.other.top / documentZoom + "px";
        otherCursor.style.height = (pos.other.bottom - pos.other.top) * .85 / documentZoom + "px";
      }
    }
    ...
    function drawSelectionRange(cm, range, output) {
      ...
      function add(left, top, width, bottom) {
        ...
        fragment.appendChild(elt("div", null, "CodeMirror-selected", "position: absolute; left: " + left / documentZoom +
          "px; top: " + top / documentZoom + "px; width: " + (width == null ? rightSide - left : width) / documentZoom +
          "px; height: " + (bottom - top) / documentZoom + "px"));
      }
    ...
    function Widget(completion, data) {
      ...
      if (overlapY > 0) {
        ...
        if (curTop - height > 0) { // Fits above cursor
          hints.style.top = (top = pos.top - height - offsetTop) / documentZoom + "px";
          below = false;
        } else if (height > winH) {
          hints.style.height = (winH - 5) / documentZoom + "px";
          hints.style.top = (top = pos.bottom - box.top - offsetTop) / documentZoom + "px";
          var cursor = cm.getCursor();
          if (data.from.ch != cursor.ch) {
            pos = cm.cursorCoords(cursor);
            hints.style.left = (left = pos.left - offsetLeft) / documentZoom + "px";
      ...
      if (overlapX > 0) {
        if (box.right - box.left > winW) {
          hints.style.width = (winW - 5) / documentZoom + "px";
          overlapX -= (box.right - box.left) - winW;
        }
        hints.style.left = (left = pos.left - overlapX - offsetLeft) / documentZoom + "px";
      }
      if (scrolls) for (var node = hints.firstChild; node; node = node.nextSibling)
        node.style.paddingRight = cm.display.nativeBarWidth / documentZoom + "px"
      ...
      cm.on("scroll", this.onScroll = function () {
        ...
        hints.style.top = newTop / documentZoom + "px";
        hints.style.left = (left + startScroll.left - curScroll.left) / documentZoom + "px";
      });
